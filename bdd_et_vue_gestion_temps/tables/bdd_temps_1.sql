...
SELECT
    ...
    DATE_ADD(entree, interval (8 + heure_pause)*60 minute) as debut,
    case
...
SELECT
    ...
    case
        when entree < time '08:00:00'
        then entree
        when (SELECT var_fin) > DATE_ADD(time '24:00:00',interval 5 hour)
        then time '05:00:00'
        else (SELECT var_debut)
    end as debut,
    CASE
        when entree < time '08:00:00'
        then time '08:00:00'
...
select id,debut,fin,duree,jour
from
...
select id,debut,fin,duree from heure_supplementaire_nuit;
